...
test_that("gbtm_predict warns when a fit has a degenerate empty group", {
  ...
  cspec <- gbtm_spec(sim_continuous, paste0("y", 1:10),
                     paste0("t", 1:10), id = "id", family = "gaussian")
...
test_that("run_gbtm_pipeline runs end-to-end and returns a full result", {
  ...
  # candidate range, degree search fixed at 1 (single shape). Group-number
  # selection uses degree 2: the planted shapes are curved, so linear-only
  # selection under-selects.
