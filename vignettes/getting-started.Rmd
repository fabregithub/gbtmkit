...
to interchangeable backends -- `trajeR` (the default), `flexmix`, `lcmm`, and
a built-in native engine (`"gbtmkit"`) -- behind one interface: every fitting
function takes an `engine` argument, and the fit diagnostics (entropy, APPA,
OCC, group proportions) are computed the same way regardless of engine, for
binary and continuous outcomes alike.
...
local optimum with multi-start initialization (`n_starts`), and predict group
...
trajeR offers `"L"`, `"EM"` and `"EMIRLS"`; the one with the lowest BIC is
selected. (Engines with a single optimizer skip this stage.)
...
(The one-call pipeline above searched up to cubic; here we cap the search at
...
refines the per-group degrees, as above):
...
optimizer does exactly that: one group comes out empty.
...
The standard defense is multi-start initialization: `n_starts` re-fits the
model from several starting points and keeps the best BIC. (For trajeR, whose
default initialization is deterministic, the extra starts come from k-means
partitions of the subjects' outcome vectors; flexmix re-runs its random EM
initialization; lcmm delegates to `lcmm::gridsearch()`.)
