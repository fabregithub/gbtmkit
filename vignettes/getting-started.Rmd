...
* **Engine-agnostic.** The pipeline talks only to a small set of accessors
  ...
  backends can be added without changing the workflow. Besides trajeR, a
  `flexmix` backend ships today: pass `engine = "flexmix"` to any fitting
  function. flexmix's EM is fast, but it fits one polynomial order shared by
  all groups, so the shape search sweeps uniform shapes there.
