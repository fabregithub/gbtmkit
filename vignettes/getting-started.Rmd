...
apply the GRoLTS acceptance criteria (PMS > 0.05, APPA > 0.70, OCC >= 5):
...
We fit with `method = "EM"` here. For this censored-normal problem the default
`"L"` (maximum-likelihood) optimizer lands in a degenerate local optimum where
one group ends up empty -- its trajectory is then unconstrained and looks
extreme. `"EM"` recovers all four groups with a much lower BIC, and is exactly
what [`select_algorithm()`](../reference/select_algorithm.html) would choose. If
a group ever does come out empty, `plot_trajectories()` / `gbtm_predict()` warn
you.
