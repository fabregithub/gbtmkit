...
- New built-in estimation engine: `engine = "gbtmkit"` – a clean-room,
  ...
  tolerance (`reltol`), native k-means multi-start, and standard errors
  via `hessian = TRUE` (`fit$vcov`). Censored-normal bounds (ymin/ymax)
  are not supported yet – use trajeR for those. trajeR remains the
  default engine; the three established packages stay available as
  citable instruments.
